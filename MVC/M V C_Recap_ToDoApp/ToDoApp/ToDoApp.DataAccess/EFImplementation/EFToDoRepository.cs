...
namespace ToDoApp.DataAccess.EFFImplementation
{
    public class EFToDoRepository : IRepository<ToDo>
    {
        // the repository uses db context to interact with the db using EF
        ...
        { //SQL:
            ...
            var todo = _dbContext.ToDo.Include(x => x.Status) // join
                .Include(x => x.Category) // join
                .FirstOrDefault(t => t.Id == id); // get the first ToDo with the specified id
            return todo;
        }
