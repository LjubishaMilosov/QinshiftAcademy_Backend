...
{
    // defines and configures the database context for our entities
    public class ToDoAppDbContext : DbContext
    {
        // Define database tables
        // DbSet<T> is a collection of entities of type T that can be queried
        // It corresponds to a table in b, where the properties are columns
        // here we a re telling the db context that out of all our classes, only these three should be entities that will be tables in our db
        ...
        // override the OnModelCreating method that is used for configurating the relaionships, constraints, seeding data, etc.
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            // one to many -> Each ToDo has one status, each status can have many ToDos
            modelBuilder.Entity<ToDo>()
                .HasOne(t => t.Status) // each ToDo has one Status
                .WithMany() // each Status can have many ToDos
                .HasForeignKey(t => t.StatusId); // the foreign key in ToDo is StatusId

            // one to many -> Each ToDo has one Category, each category can have many ToDos
            modelBuilder.Entity<ToDo>()
                .HasOne(t => t.Category) // each ToDo has one Status
                .WithMany() // each Category can have many ToDos
                .HasForeignKey(t => t.CategoryId); // the foreign key in ToDo is CategoryId
